<x-filament-panels::page>
    ...
    <div class="mb-6 sm:mb-8 overflow-x-auto">
        <div class="flex items-center justify-between min-w-[400px] max-w-3xl mx-auto px-2">
            @php
                $steps = [
            ...
            @foreach($steps as $stepNum => $step)
                <div class="flex items-center {{ $stepNum < 5 ? 'flex-1' : '' }}">
                    <div class="flex flex-col items-center">
                        <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0
                        ...
                        <span class="text-[10px] sm:text-xs mt-1 text-center leading-tight {{ $currentStep >= $stepNum ? 'text-primary-600 font-medium' : 'text-gray-400' }}">
                            {{ $step['label'] }}
                        </span>
                    </div>
                    @if($stepNum < 5)
                        <div class="flex-1 h-0.5 mx-1 sm:mx-2 {{ $currentStep > $stepNum ? 'bg-primary-600' : 'bg-gray-200' }}"></div>
                    @endif
                @endforeach
            </div>
        </div>
        ...
        <x-filament::section>
            <x-slot name="heading">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm sm:text-base">Langkah 1: Pilih Tahun Fiskal</span>
                </div>
            </x-slot>

            @if(count($openFiscalYears) > 0)
                <p class="text-gray-600 mb-4 text-sm sm:text-base">
                    Pilih tahun fiskal yang ingin ditutup.
                </p>

                <div class="space-y-3">
                    @foreach($openFiscalYears as $fy)
                        <label class="flex items-center p-3 sm:p-4 border rounded-lg cursor-pointer transition
            ...
            <x-slot name="heading">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 w-full">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm sm:text-base">Langkah 2: Neraca Saldo {{ $this->getSelectedYear() }}</span>
                    </div>
                    @if($tbBalanced)
            ...
            @if(!$canClose)
                <div class="bg-red-50 border border-red-200 rounded-lg p-3 sm:p-4 mb-4">
                    <div class="flex items-center gap-2 text-red-800">
                        ...
                        <span class="font-medium text-sm">{{ $canCloseReason }}</span>
                    </div>
                </div>
            @else
                    ...
                    <table class="w-full text-sm min-w-[500px]">
                        <thead>
                            <tr class="border-b bg-gray-50">
                                ...
                                <th class="px-3 py-2 text-left hidden sm:table-cell">Tipe</th>
        ...
        <x-filament::section>
            <x-slot name="heading">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm sm:text-base">Langkah 3: Laba/Rugi {{ $this->getSelectedYear() }}</span>
                </div>
            </x-slot>

            {{-- Mobile: stacked, Desktop: side by side --}}
            <div class="space-y-6 md:grid md:grid-cols-2 md:gap-6">
                {{-- Revenue --}}
                <div>
                    <h4 class="font-semibold text-green-700 mb-2 text-sm sm:text-base">📈 Pendapatan</h4>
                ...
                {{-- Expense --}}
                <div>
                    <h4 class="font-semibold text-red-700 mb-2 text-sm sm:text-base">📉 Beban</h4>
            ...
            {{-- Net Income Summary --}}
            <div class="mt-6 p-3 sm:p-4 rounded-lg {{ $incomeNetIncome >= 0 ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200' }}">
                <div class="flex items-center justify-between">
                    <span class="font-semibold text-base sm:text-lg">
                        {{ $incomeNetIncome >= 0 ? '💰 Laba Bersih' : '📉 Rugi Bersih' }}
                    </span>
                    <span class="font-bold text-lg sm:text-xl {{ $incomeNetIncome >= 0 ? 'text-green-700' : 'text-red-700' }}">
        ...
        <x-filament::section>
            <x-slot name="heading">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm sm:text-base">Langkah 4: Konfirmasi Tutup Buku</span>
                </div>
            </x-slot>

            <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 sm:p-4 mb-6">
                <div class="flex items-center gap-2 text-amber-800">
                    <x-heroicon-o-exclamation-triangle class="w-5 h-5 shrink-0" />
                    <span class="font-medium text-sm">Peringatan: Tindakan ini tidak dapat dibatalkan!</span>
                </div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6">
                <div class="bg-white border rounded-lg p-3 sm:p-4 text-center">
                    <div class="text-xs sm:text-sm text-gray-500">Tahun Fiskal</div>
                    <div class="text-xl sm:text-2xl font-bold text-gray-900">{{ $this->getSelectedYear() }}</div>
                </div>
                <div class="bg-white border rounded-lg p-3 sm:p-4 text-center">
                ...
                <div class="bg-white border rounded-lg p-3 sm:p-4 text-center">
                    <div class="text-xs sm:text-sm text-gray-500">Total Beban</div>
            ...
            <div class="bg-gray-50 border rounded-lg p-3 sm:p-4 mb-6">
                <h4 class="font-semibold mb-2 text-sm sm:text-base">Yang akan dilakukan:</h4>
                <ul class="space-y-2 text-xs sm:text-sm text-gray-700">
                    <li class="flex items-center gap-2">
                        <x-heroicon-s-check class="w-4 h-4 text-green-500 shrink-0" />
        ...
        <x-filament::section>
            <div class="text-center py-6 sm:py-8">
                <div class="w-16 h-16 sm:w-20 sm:h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <x-heroicon-s-check-circle class="w-10 h-10 sm:w-12 sm:h-12 text-green-600" />
                </div>

                <h3 class="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Tutup Buku Berhasil! 🎉</h3>
                <p class="text-gray-600 mb-6 text-sm sm:text-base">{{ $closingResult['message'] ?? 'Tahun fiskal berhasil ditutup.' }}</p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 max-w-2xl mx-auto mb-6">
                    <div class="bg-white border rounded-lg p-3 sm:p-4">
                        <div class="text-xs sm:text-sm text-gray-500">Tahun Ditutup</div>
                        <div class="text-lg sm:text-xl font-bold">{{ $closingResult['year'] ?? '-' }}</div>
                    </div>
                    <div class="bg-white border rounded-lg p-3 sm:p-4">
                        <div class="text-xs sm:text-sm text-gray-500">Laba Bersih</div>
                        <div class="text-lg sm:text-xl font-bold {{ ($closingResult['netIncome'] ?? 0) >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            Rp {{ number_format(abs($closingResult['netIncome'] ?? 0), 0, ',', '.') }}
                        </div>
                    </div>
                    <div class="bg-white border rounded-lg p-3 sm:p-4">
                        <div class="text-xs sm:text-sm text-gray-500">Tahun Baru</div>
                        <div class="text-lg sm:text-xl font-bold text-primary-600">{{ $closingResult['nextYear'] ?? '-' }}</div>
                    </div>
                </div>
